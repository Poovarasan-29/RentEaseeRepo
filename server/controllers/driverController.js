...
const storage = multer.diskStorage({
    ...
    filename: (req, file, cb) => {
        ...
        switch (file.fieldname) {
            case 'aadherCardDoc':
                path = { [file.fieldname]: '/Images/Driver-Aadhar-Documents/' + Date.now() + file.originalname }
                break;
            case 'licenceCardDoc':
                path = { [file.fieldname]: '/Images/Driver-Licence-Documents/' + Date.now() + file.originalname }
                break;
            case 'expiranceDoc':
                path = { [file.fieldname]: '/Images/Driver-Experiance-Documents/' + Date.now() + file.originalname }
                break;
            case 'driverPhoto':
                path = { [file.fieldname]: '/Images/Driver-Profile-Documents/' + Date.now() + file.originalname }
                break;
            default:
                break;
        }
...
const uploadHandler = upload.fields([{ name: 'aadherCardDoc', maxCount: 1 }, { name: 'licenceCardDoc', maxCount: 1 }, { name: 'expiranceDoc', maxCount: 1 }, { name: 'driverPhoto', maxCount: 1 }])
...
exports.getNewDriverDetails = (req, res) => {
    ...
    uploadHandler(req, res, async (err) => {
        if (err instanceof multer.MulterError) {
            ...
            const driverRegisteredIDs = await driverdetails.find({}, { driverID: 1, _id: 0 });
...
exports.getDriverFullDetails = async (req, res) => {
    try {
        ...
        const relatedDrivers = await driverdetails.find({ district: driverDetails[0].district, driverID: { $ne: id } });
...
exports.createNewDriver = async (req, res) => {
    const { imageUrls, inputs, selectedDistrict, selectedCity } = req.body;
    const values = inputs;
