...
// import { Helmet } from "react-helmet-async";
// import { useSelector } from "react-redux";
// import CarDetailsForm from "../components/newcar/CarDetailsForm";
// import LocationDetailsForm from "../components/newcar/LocationDetailsForm";
// import PricingDetailsForm from "../components/newcar/PricingDetailsForm";
// import ImageUpload from "../components/newcar/ImageUpload";
// import DescriptionAndContactForm from "../components/newcar/DescriptionAndContactForm";
// import PostCarButton from "../components/newcar/PostCarButton";
...
//     const [check, setCheck] = useState(false);
//     const [loading, setLoading] = useState(false);
//     const [files, setFiles] = useState({ carPhotos: [], insuranceDocument: null });
//     const navigate = useNavigate();
...
//                     return;
//                 }
//                 setFiles((prevFiles) => ({ ...prevFiles, [name]: Array.from(selectedFiles) }));
//                 const combineImagesForDisplay = [];
//                 for (let i = 0; i < selectedFiles.length; i++) {
//                     combineImagesForDisplay.push(URL.createObjectURL(selectedFiles[i]));
//                 }
...
//                 console.error('Failed to upload all files:', error);
//                 setLoading(false);
//                 toast.error("Failed to upload files", { autoClose: 1200 });
//             });
//     };
...
//             } else if (name === "state") {
//                 const districtsWithCity = regionData.regions.filter(state => state.state === value)[0].districts;
//                 setInputs((prev) => ({ ...prev, [name]: value, district: '', city: '' }));
//                 setAllDistricts(districtsWithCity);
//             } else if (name === "district") {
//                 const citiesWithDistrict = allDistricts.filter(dist => dist.district === value)[0].city;
//                 setInputs((prev) => ({ ...prev, [name]: value, city: '' }));
//                 setAllCities(citiesWithDistrict);
//             }
//         }
//     }
...
//                         <legend className="fw-bold">Vehicle Information</legend>
//                         <CarDetailsForm
//                             inputs={inputs}
//                             setInputs={setInputs}
//                             brands={brands}
...
//                     </fieldset>
//                     <fieldset className="col-12">
//                         <legend className="fw-bold">Location Details</legend>
//                         <LocationDetailsForm
//                             inputs={inputs}
//                             setInputs={setInputs}
//                             allStates={allStates}
...
//                     </fieldset>
//                     <fieldset className="col-12">
//                         <legend className="fw-bold">Pricing</legend>
//                         <PricingDetailsForm
//                             inputs={inputs}
//                             setInputs={setInputs}
//                             handleInputs={handleInputs}
...
//                     </fieldset>
//                     <fieldset className="col-12">
//                         <legend className="fw-bold">Media</legend>
//                         <ImageUpload
//                             files={files}
//                             setFiles={setFiles}
...
//                     </fieldset>
//                     <fieldset className="col-12">
//                         <legend className="fw-bold">Description & Contact</legend>
//                         <DescriptionAndContactForm
//                             inputs={inputs}
//                             setInputs={setInputs}
//                             handleInputs={handleInputs}
//                         />
//                     </fieldset>
//                     <PostCarButton loading={loading} handleUpload={handleUpload} />
//                 </form>
//             </div>
//         </>
//     );
...
export default function NewCar() {
    const { userID } = useSelector(state => state.checkUserLoginSlice);
    const [inputs, setInputs] = useState({ brand: '', model: '', manufacturedYear: '', fuelType: '', fuelCapacity: '', transmission: '', ac: '', vechileNo: '', depositAmountDay: '', depositAmountMonth: '', rentAmountDay: '', rentAmountMonth: '', carLocation: '', milage: '', noOfSeats: '', state: '', district: '', city: '', description: '', phoneNo: '' });
    const [displayImages, setDisplayImages] = useState([]);
    const [brands, setBrands] = useState([]);
    const [models, setModels] = useState([]);
    const [allStates, setAllStates] = useState(regionData.regions.map(region => region.state));
    ...
    const getBrands = () => carsData.cars.map(car => car.brand).sort();

    const handleFileChange = (e) => {
        if (!userID) {
            toast.warning("Login first", { autoClose: 750 });
            setTimeout(() => navigate('/renteasee/login'), 1000);
        } else {
            const { name, files: selectedFiles } = e.target;
            if (name === 'carPhotos') {
                if (selectedFiles.length !== 5) {
                ...
                setDisplayImages(prev => [...prev, ...Array.from(selectedFiles).map(file => URL.createObjectURL(file))]);
            } else {
    ...
    const handleUpload = async (e) => {
        ...
        for (const [key, file] of Object.entries(files)) {
            if (file) {
                if (Array.isArray(file)) {
                    const carPhotoUploads = file.map((photo, index) => uploadFile(photo, `carPhotos/${index + 1}_${photo.name}`));
                    uploadPromises.push(Promise.all(carPhotoUploads).then(urls => uploadResults[key] = urls));
                } else {
                    uploadPromises.push(uploadFile(file, `${key}/${file.name}`).then(url => uploadResults[key] = url));
                }
            }
        }
    ...
    const handleInputs = (e) => {
        if (!userID) {
            ...
            setInputs(prev => ({ ...prev, [name]: value }));
            if (name === "brand") {
                const foundBrand = carsData.cars.find(car => car.brand === value);
                setModels(foundBrand?.models || []);
            } else if (name === "state") {
                const districtsWithCity = regionData.regions.find(state => state.state === value)?.districts || [];
                setInputs(prev => ({ ...prev, [name]: value, district: '', city: '' }));
                setAllDistricts(districtsWithCity);
            } else if (name === "district") {
                const citiesWithDistrict = allDistricts.find(dist => dist.district === value)?.city || [];
                setInputs(prev => ({ ...prev, [name]: value, city: '' }));
                setAllCities(citiesWithDistrict);
            }
        }
    ...
    return (
        <>
            ...
            <Container maxWidth="md" sx={{ mt: 16, mb: 6 }}>
                <Typography variant="h4" align="center" color="#2c3e50" fontWeight={'bold'} marginBottom={'2rem'} gutterBottom>
                    List Your Vehicle
                </Typography>


                {loading && (
                    <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', display: 'flex', justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0, 0, 0, 0.5)', zIndex: 1200, width: '100vw', height: '100vh' }}>
                        <CircularProgress />
                    </div>
                )}
                <Paper elevation={3} sx={{ p: 3, boxShadow: '1px 1px 10px 0px rgba(0,0,0,.1)' }}>
                    <Grid container spacing={3}>
                        <Grid item xs={12} md={6}>
                            <CarDetailsForm inputs={inputs} setInputs={setInputs} brands={brands} models={models} handleInputs={handleInputs} />
                        </Grid>
                        <Grid item xs={12} md={6}>
                            <LocationDetailsForm inputs={inputs} setInputs={setInputs} allStates={allStates} allDistricts={allDistricts} allCities={allCities} handleInputs={handleInputs} />
                        </Grid>
                        <Grid item xs={12} md={6}>
                            <PricingDetailsForm inputs={inputs} setInputs={setInputs} handleInputs={handleInputs} />
                        </Grid>
                        <Grid item xs={12} md={6}>
                            <DescriptionAndContactForm inputs={inputs} setInputs={setInputs} handleInputs={handleInputs} />
                        </Grid>
                        <Grid item xs={12}>
                            <ImageUpload files={files} setFiles={setFiles} displayImages={displayImages} setDisplayImages={setDisplayImages} handleFileChange={handleFileChange} check={check} />
                        </Grid>
